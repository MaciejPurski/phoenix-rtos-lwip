...
static int socket_ioctl6(int sock, unsigned long request, const void *in_data, const void *out_data)
{
	switch (request) {
...
static int do_getifaddrs(void *buf, size_t *buflen)
{
	const size_t sin_alligned = (sizeof(struct sockaddr_in) + sizeof(size_t) - 1) & ~(sizeof(size_t) - 1);
...
#if LWIP_IPV6
	const size_t sin6_alligned = (sizeof(struct sockaddr_in6) + sizeof(size_t) - 1) & ~(sizeof(size_t) - 1);
	...
	*buflen = needed;
	...
	NETIF_FOREACH(netif) {
		...
		dest->ifa_addr = (void *) (addrdest - buf);
		addrdest += sin_alligned;

		inet_addr_from_ip4addr(&sin->sin_addr, netif_ip4_gw(netif));
		memcpy(addrdest, sin, sin->sin_len);
		dest->ifa_dstaddr = (void *) (addrdest - buf);
		addrdest += sin_alligned;

		inet_addr_from_ip4addr(&sin->sin_addr, netif_ip4_netmask(netif));
		memcpy(addrdest, sin, sin->sin_len);
		dest->ifa_netmask = (void *) (addrdest - buf);
		addrdest += sin_alligned;

		snprintf(strdest, sizeof(netif->name) + 2, "%2s%1d", netif->name, netif->num);
		dest->ifa_name = (void *)(strdest - buf);
#if LWIP_IPV6
		sin6 = (struct sockaddr_in6 *) &sa;
		for (i = 0; i < LWIP_IPV6_NUM_ADDRESSES; i++) {
			if (!ip6_addr_isinvalid(netif_ip6_addr_state(netif, i))) {
				dest->ifa_next = (void *)((void *)(dest + 1) - buf);
				++dest;
		...
		dest->ifa_next = netif->next ? (void *)((void *)(dest + 1) - buf) : NULL;
		++dest;
	}
