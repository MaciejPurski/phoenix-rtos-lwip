...
loadng_g3_link_cost_dir(struct netif *netif, u8_t mod_type, u8_t active_tones, u8_t lqi)
{
  ...
  kq_part = (kq_part > (ctx->high_lqi_value - ctx->low_lqi_value)) ?
            ctx->kq : ctx->kq * kq_part / (ctx->high_lqi_value - ctx->low_lqi_value);

  kc_part = ctx->kc * (ctx->max_tones - active_tones) / ctx->max_tones;
